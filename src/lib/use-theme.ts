...
// página. El header monta 2 toggles (desktop `md:flex` + mobile `md:hidden`): si
// no se sincronizan, togglear uno deja al otro con el estado viejo y al cambiar de
...
const THEME_EVENT = 'market-theme-change'
...
export function useTheme() {
  ...
  const toggle = useCallback((_origin?: RevealOrigin) => {
    ...
    // (flushSync) para que todo —shell, grilla, chart— quede en la misma "foto".
    const apply = () => {
      applyTheme(next)
      flushSync(() => setThemeState(next))
      // Notifica a las demás instancias del hook (el otro toggle del header) para
      // que no queden desincronizadas al cambiar de breakpoint.
    ...
    // Todo el cambio (shell, grilla, chart) ocurre dentro de la misma foto.
